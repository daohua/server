...
from . import serializers, app_settings, utils
...
class RegistrationView(utils.SendEmailViewMixin, generics.CreateAPIView):
    ...
    def get_serializer_class(self):
        if app_settings.get('LOGIN_AFTER_REGISTRATION'):
    ...
    def post_save(self, obj, created=False):
        ...
        if app_settings.get('SEND_ACTIVATION_EMAIL'):
    ...
    def get_email_context(self, user):
        ...
        context['url'] = app_settings.get('ACTIVATION_URL').format(**context)
        return context
...
class PasswordResetView(utils.ActionViewMixin, utils.SendEmailViewMixin, generics.GenericAPIView):
    ...
    def get_email_context(self, user):
        ...
        context['url'] = app_settings.get('PASSWORD_RESET_CONFIRM_URL').format(**context)
        return context
...
class SetPasswordView(utils.ActionViewMixin, generics.GenericAPIView):
    ...
    def get_serializer_class(self):
        if app_settings.get('SET_PASSWORD_RETYPE'):
...
class PasswordResetConfirmView(utils.ActionViewMixin, generics.GenericAPIView):
    ...
    def get_serializer_class(self):
        if app_settings.get('PASSWORD_RESET_CONFIRM_RETYPE'):
...
class ActivationView(utils.ActionViewMixin, generics.GenericAPIView):
    ...
    def action(self, serializer):
        ...
        serializer.user.save()
        if app_settings.get('LOGIN_AFTER_ACTIVATION'):
...
class SetUsernameView(utils.ActionViewMixin, generics.GenericAPIView):
    ...
    def get_serializer_class(self):
        if app_settings.get('SET_USERNAME_RETYPE'):
