...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    def save(self, **kwargs):
        print "here is the data %s" % self.init_data.items()
        print "here is the files %s" % self.init_files.items()
        self.object = User.objects.create_user(**dict(self.init_data.items() 
                                                      + self.init_files.items()))
        return self.object
...
class UserLoginSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ('email', 'avatar', 'auth_token')
        write_only_fields = (
            'password',
        )
